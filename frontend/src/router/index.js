...
router.beforeEach((to, from, next) => {
  if (
		...
		next('/auth/login') // if the authorized getter returns true, you can continue on to the current route. to.name
  } else if ((to.name === 'login' || to.name === 'registration') && store.getters.authorized) {
		next('/') //sends to login if auth token isnt true
  } else if (
        (to.name === 'cargoes' || to.name === 'filials' || to.name === 'locations' || to.name === 'cargo_connected') &&
