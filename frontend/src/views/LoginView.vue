            ...
            <v-btn plain>
              <router-link to="registration">
                  Sign up
						...
						<v-spacer />
						<v-btn align-center justify-center :disabled="!phone || !password" color="tertiary" @click="login"> Login </v-btn>
					</v-card-actions>
...
<script>
	export default {
      ...
      login: function () {
        let phone = this.phone // you use this.username to access the username saved in the data
        let password = this.password
        this.$store
            .dispatch('login', {phone, password}) //calls the login action and passes login info
            .then(() => {
              console.log("auth")
              this.$router.push('/')
